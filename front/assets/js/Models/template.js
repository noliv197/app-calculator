export default class Template{
    ...
    static descriptionTemplate(data){
        return `
            <h2 class="description__title">${data.title}</h2>
            <p class="description__text">${data.description}</p>
            <br>
            <p class="description__equation">${data.equation}</p>    
        `
    }
    static solutionsTemplate(data){
        return `
    ...
    static conversorTemplate(data){
        return `
            <h3 class="programm__title">${data.name}</h3>
            <div class="programm__form--conversor">
                <fieldset class="form--conversor" data-text="field-1">
                    ${new Template().conversorInputTemplate(data)}
                </fieldset>
                ...
                <fieldset class="form--conversor" data-text="field-2">
                    ${new Template().conversorOuputTemplate(data)}
                </fieldset>
            </div>
        `
    }

    conversorInputTemplate(data){
        return `
